...
- `internal/mcp`: optional Model Context Protocol server (added v0.1.19, opt-in). Exposes 21 tools as thin adapters over `services.AppService` over two transports: an HTTP listener on `:8081` (started when a token is resolved — from `SHELLYADMIN_MCP_TOKEN`, or from the persisted settings since v0.1.20 — and off by default; bearer-token gated), and `shellyctl mcp` over stdio for a client on the same host (v0.2.3). **13 of the tools are read-only and 8 change device state** — the latter are confirm-gated: without `confirm: true` they return a preview and do not call the underlying method (v0.1.22). Nothing in the surface mutates ShellyAdmin's own config. Design rationale, and the addenda that added the state-changing tools and the stdio transport, in [adr/0011-mcp-read-only-server.md](./adr/0011-mcp-read-only-server.md); the agent threat model in [adr/0014-mcp-agent-threat-model.md](./adr/0014-mcp-agent-threat-model.md).
