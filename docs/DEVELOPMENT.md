...
**Toolchain alignment:** CI's `setup-go` / `setup-node` must match the `golang:` / `node:` tags in `docker/Dockerfile`, and the `go.mod` directive is the floor — it may lag the toolchain but must never outrank it. The `Toolchain sync` job enforces all three; read the current numbers there, not here. Being *required* is what makes it useful against Dependabot: a base-image bump is `semver-minor`, so without a failing required check `gh pr merge --auto` would land it with CI still on the old toolchain — which is why there is no `ignore` rule for `golang` in `.github/dependabot.yml`.

`golangci-lint` is the one link the job cannot check — nothing machine-readable ties a release to the Go it was built with — and it must be built with a Go ≥ the toolchain it analyzes: v2.6 (built with 1.25) panicked on the 1.26 stdlib, v2.12 (built with 1.26) on the 1.27 stdlib, both with `file requires newer Go version`. It has to be bumped by hand alongside `go-version`; the comment on the lint step carries the detail. Because the drift check now forces `setup-go` to move in the same PR as the image, that panic surfaces where it belongs instead of in an unrelated PR (as it did in #110).
