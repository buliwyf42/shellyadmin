...
| `SHELLYADMIN_ENCRYPTION_KEY` / `_FILE` | base64-encoded 32-byte key for credential at-rest encryption                   | **Required** since v0.3.0 (ADR-0013) — the binary refuses to start without it. Manage it externally (Docker secret, sops, NixOS secret); prefer the `_FILE` form. Keep it stable across recreates — losing it orphans every stored credential. To switch to a new key deliberately, stop the container and run `shellyctl rotate-key` (v0.5.3; new key via `SHELLYADMIN_NEW_ENCRYPTION_KEY[_FILE]`, dry-run by default — see [SECURITY.md](SECURITY.md)). |
