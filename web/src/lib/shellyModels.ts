// SKU -> marketing name lookup, sourced from aioshelly (Home Assistant's
// official Shelly library, github.com/home-assistant-libs/aioshelly,
// aioshelly/const.py MODEL_NAMES, checked 2026-07-22) — Shelly's own docs
// don't publish a SKU table. Display-only; never used for gating logic.
...
/** Marketing name for a Shelly model SKU (e.g. "SNSW-001X16EU" -> "Shelly
 * Plus 1"), or undefined when the SKU isn't in the table (new/unlisted
 * model). */
export function modelName(sku: string | null | undefined): string | undefined {
  if (!sku) return undefined;
  return MODEL_NAMES[sku];
}
