...
<section class="page-hero">
  <div class="page-hero-stack">
    <button class="btn btn-outline-light btn-sm mb-2" on:click={() => navigate('/')}>Back to Devices</button>
    <h1 class="h4 mb-0">{detail?.device.name || detail?.device.serial || detail?.device.mac || target}</h1>
      ...
      <div class="card bg-dark border-secondary mb-3">
        <div class="card-body">
          ...
          <div class="detail-grid">
            <div><span class="text-secondary">Model</span><strong>{detail.device.model || 'n/a'}</strong></div>
            <div><span class="text-secondary">Firmware</span><strong>{detail.device.fw || 'n/a'}</strong></div>
            <div><span class="text-secondary">Generation</span><strong>Gen {detail.device.gen}</strong></div>
            <div><span class="text-secondary">Online</span><strong>{detail.device.online ? 'Yes' : 'No'}</strong></div>
            <div>
              <span class="text-secondary">Last Success</span>
              <strong title={detail.device.last_seen ? formatDateTime(detail.device.last_seen) : 'never'}>
                {detail.device.last_seen ? formatRelativeDateTime(detail.device.last_seen) : 'never'}
              </strong>
            </div>
            <div><span class="text-secondary">Auth</span><strong>{detail.device.auth_required ? detail.device.auth_error || 'required' : 'clear'}</strong></div>
          </div>
        </div>
      </div>
          ...
          <div class="d-flex flex-column gap-2">
            {#each detail.actions as action}
              <div class="action-row">
                ...
                <button class="btn btn-sm {action.risk === 'high' ? 'btn-warning text-dark' : 'btn-outline-light'}" disabled={!action.supported || busyAction === action.id} on:click={() => runAction(action.id)}>
    ...
    <div class="col-lg-7">
      <div class="card bg-dark border-secondary mb-3">
        <div class="card-body">
          ...
          {:else}
            <div class="alert alert-secondary mb-0">No compliance issues detected in the latest snapshot.</div>
          {/if}
        </div>
      </div>
