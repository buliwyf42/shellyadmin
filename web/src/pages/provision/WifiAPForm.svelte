...
>
  <div class="sa-form-grid">
    ...
    <div data-span="4">
      <FieldRow
        label="Open AP (no password)"
        bind:enabled={state.isOpenField}
        disabled={!state.enabled}
      >
        <Toggle
          bind:checked={state.isOpen}
          disabled={!state.enabled || !state.isOpenField}
    ...
    <div data-span="4">
      <FieldRow label="SSID" bind:enabled={state.ssidEnabled} disabled={!state.enabled}>
        <input
          class="form-control"
          bind:value={state.ssid}
          disabled={!state.enabled || !state.ssidEnabled}
        />
      </FieldRow>
    </div>
    <div data-span="4">
