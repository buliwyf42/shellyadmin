<script lang="ts">
  ...
  $: expanded = state.rpcEnabledField || state.observerEnabledField;
</script>

<SectionCard tag="ble" title="Bluetooth (Gen 2+)" bind:open={state.open} forceOpen={expanded}>
  ...
  <div class="sa-form-grid">
    <div data-span="6">
      <FieldRow label="Enable RPC over BLE" bind:enabled={state.rpcEnabledField}>
        <Toggle
          bind:checked={state.rpcEnabled}
          disabled={!state.rpcEnabledField}
          label={state.rpcEnabled ? 'On' : 'Off'}
        />
      </FieldRow>
    </div>
    <div data-span="6">
      <FieldRow label="Observer Mode" bind:enabled={state.observerEnabledField}>
        <Toggle
          bind:checked={state.observerEnabled}
          disabled={!state.observerEnabledField}
