  ...
  <div class="sa-form-grid">
    <div data-span="4">
      <FieldRow label="Enable Matter" bind:enabled={matter.enableField} disabled={!matter.enabled}>
        <Toggle bind:checked={matter.enable} disabled={!matter.enabled || !matter.enableField} label={matter.enable ? 'On' : 'Off'} />
      </FieldRow>
    </div>
  </div>
</SectionCard>

<SectionCard tag="cloud" title="Shelly Cloud" bind:open={cloud.open} forceOpen={cloudExpanded} bind:enabled={cloud.enabled}>
  <div class="sa-form-grid">
    <div data-span="4">
      <FieldRow label="Enable Cloud" bind:enabled={cloud.enableField} disabled={!cloud.enabled}>
        <Toggle bind:checked={cloud.enable} disabled={!cloud.enabled || !cloud.enableField} label={cloud.enable ? 'On' : 'Off'} />
      </FieldRow>
    </div>
  </div>
</SectionCard>

<SectionCard tag="ota" title="Firmware Update" bind:open={ota.open} forceOpen={otaExpanded} bind:enabled={ota.enabled}>
  <div class="sa-form-grid">
    <div data-span="4">
      <FieldRow label="Stage" bind:enabled={ota.stageEnabled} disabled={!ota.enabled}>
        <Select bind:value={ota.stage} options={stageOptions} disabled={!ota.enabled || !ota.stageEnabled} ariaLabel="Stage" />
      </FieldRow>
    </div>
    <div data-span="6">
      <FieldRow
...
<SectionCard tag="auth" title="Set Device Password (Gen 2+)" bind:open={auth.open} forceOpen={authExpanded} bind:enabled={auth.enabled}>
  <div class="sa-form-grid">
    <div data-span="6">
