...
<SectionCard tag="ws" title="WebSocket (Gen 2+)" bind:open={state.open} forceOpen={expanded} bind:enabled={state.enabled}>
  <div class="sa-form-grid">
    ...
    <div data-span="4">
      <FieldRow label="Connection type" bind:enabled={state.tlsModeEnabled} disabled={!state.enabled}>
        <Select bind:value={state.tlsMode} options={tlsModeOptions} disabled={!state.enabled || !state.tlsModeEnabled} ariaLabel="Connection type" />
      </FieldRow>
    </div>
    <div data-span="4">
  ...
  {#if state.serverEnabled && state.server && !isTLSServerURL(state.server)}
    <div class="text-secondary mt-2" style="font-size: 0.8rem;">TLS settings are ignored for plain <code>ws://</code> endpoints. Use <code>wss://</code> for TLS.</div>
  {/if}
</SectionCard>
