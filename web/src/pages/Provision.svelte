    ...
    <div class="card-body">
      <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
        ...
        {#if restartRequiredDevices.length > 0}
          ...
          >Reboot {restartRequiredDevices.length} restart-required device{restartRequiredDevices.length !== 1 ? 's' : ''}</button>
        {/if}
      </div>
      <div class="table-responsive">
              ...
              <tr>
                ...
                <td>
                  ...
                  {#if r.restart_required}
                    <span class="badge ms-1" style="background:#c89a2a;color:#fff;">restart required</span>
                  {/if}
                </td>
                <td>
                  <div class="d-flex flex-wrap gap-1">
                    {#each r.results as s}
                      <span
                        class="badge {s.status === 'ok' ? 'bg-success' : s.status === 'skipped' ? 'bg-secondary' : 'bg-danger'}"
                        title={s.detail}
                      >{s.section}</span>
                    {/each}
                  </div>
