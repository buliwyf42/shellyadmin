...
<section class="page-hero">
  ...
  <div class="page-toolbar">
    ...
    <button class="btn btn-outline-success" on:click={addAll} disabled={status.pending.length === 0}
      >Add All</button
    >
    <button
      class="btn btn-outline-warning"
      on:click={addNewOnly}
      disabled={!status.pending.some((d) => d.is_new)}>Add New Only</button
    >
  </div>
</section>
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
      <div>
        ...
        <div class="text-secondary">
          {status.found} device{status.found === 1 ? '' : 's'} found, {status.pending.filter(
            (d) => d.is_new,
          ).length} new
        </div>
      </div>
